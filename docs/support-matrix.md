...
> [!IMPORTANT]
> The matrices describe current repository source unless a row explicitly says
> “published.” The repository release record says npm
> `@vibechess/chessboard-native@0.1.0-next.1` was published from commit
> `8d3c419`. Current main contains later changes while the package manifest
> still has that version. Those later APIs are not retroactively present in the
...
| Status           | Meaning                                                                                                                                          |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| Supported        | Part of the exact current-source support contract.                                                                                               |
...
| Import                                                 | Current repository source   | Published `0.1.0-next.1`                     | Contract                                                                                                                                                   |
| ------------------------------------------------------ | --------------------------- | -------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@vibechess/chessboard-native`                         | Supported                   | Present, at the older `8d3c419` API snapshot | Primary controlled component, provider, spare piece, public types, theme/defaults, and pure helpers. Consult the API report for the exact current symbols. |
| `@vibechess/chessboard-native/pieces`                  | Supported                   | Present                                      | Focused `defaultPieceRenderers` export.                                                                                                                    |
| `@vibechess/chessboard-native/react-chessboard-compat` | Supported in current source | **Absent**                                   | Native options-name adapter for the pinned `react-chessboard` 5.10 migration surface. Wait for a later prerelease before importing it from npm.            |
...
The feature status below applies to current repository source, not automatically
to the older npm archive.
