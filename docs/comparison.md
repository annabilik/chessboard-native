...
The fixture itself is reference material and is not shipped in the native
package. The native default renderer adapts the Cburnett design from its
Wikimedia Commons source files; the fixture was also consulted as a conversion
reference and remains excluded from the package.
...
> [!IMPORTANT]
> This page describes published `0.1.0-next.2`, including the
> `react-chessboard-compat` subpath. npm `0.1.0-next.1` is an older immutable
> archive without that export. Check the exact installed version because npm's
> `next` tag moves between prereleases.
...
published `0.1.0-next.2` package. It preserves a
...
| Piece artwork             | Twelve upstream browser renderers                                                                    | A native whole-map renderer replacement                                             | Twelve native Cburnett renderers adapted from Wikimedia Commons, or a consumer whole-map replacement    | Adapt             |
